...
bitset_create (size_t capacity)
{
  ...
  bitset->size = 0;
  bitset->data = malloc (sizeof (uint32_t) * blocks);
...
void
bitset_reserve (bitset_t* bitset, size_t capacity)
{
  assert (bitset != NULL);

  if (capacity > bitset->capacity) {
    size_t old_blocks = IDX_TO_BLOCK (bitset->capacity - 1) + 1;
    size_t blocks = IDX_TO_BLOCK (capacity - 1) + 1;
    if (blocks > old_blocks) {
      bitset->data = realloc (bitset->data, sizeof (uint32_t) * blocks);
      for (; old_blocks < blocks; ++old_blocks) {
	bitset->data[old_blocks] = 0;
      }
    }
    bitset->capacity = capacity;
  }
