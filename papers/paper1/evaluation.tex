...
Each trial was repeated 30 times.
...
The trials were performed on a MacBook Pro running OS X 10.5.8 with an 2.4 GHz Intel Core 2 Duo processor with 4GB of memory.
...
(Note that the confidence intervals are present but small.)
...
From the results, we calculated the average overhead per action of the ioa++ framework when dispatching an action and found it to be 4211ns $\pm$ 31ns for the single threaded experiments and 4858ns $\pm$ 32ns for the multi-threaded experiments.
Similary, we calculated the scheduling overhead and found it to be 227ns $\pm$ 2.5ns for the single threaded experiments and 279ns $\pm$ 2.3ns for the multi-threaded experiments.
