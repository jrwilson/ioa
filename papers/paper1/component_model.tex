...
\subsection{I/O Automata\label{io_automaton}}

In this subsection we describe how the I/O automata model offers a natural realization of components and interfaces, and provides natural support for concurrency and composition.
...
Hierarchical decomposition, the modeling dual of recursive encapsulation, is an established technique for I/O automata~\cite{lynch1987hierarchical},~\cite{lynch1994atomic}.
...
Thus, the techniques for \emph{modeling} a dynamic system of I/O automata are not appropriate for \emph{implementing} a dynamic system of I/O automata.
%% Consequently, we add the ability to deal with a dynamic set of automata and interactions.

To illustrate, let a system $S=(A,B)$ be a pair consisting of a set of automata $A$ and set of bindings $B$.
...
A dynamic system implies that the system $S$ be dependent on time $t$ measured by the number of actions that have been executed.
Thus, $S(t+1)$ need not be equal to $S(t)$.
A \emph{system action} is an action that \emph{can} change the system $S$.

\paragraph*{System actions}
We define four indivisible operations for managing a dynamic system of automata: \emph{create}, \emph{bind}, \emph{unbind}, and \emph{destroy}.
...
A bind fails if the automaton associated with the output action doesn't exist, the automaton associated with the input action doesn't exist, or any of the binding rules outlined in Section~\ref{io_automaton} would be violated.
...
Thus, the set of actions implied by any action is the set defined in Section~\ref{io_automata} \emph{and} and the system automaton.
However, the opportunities for concurrency explored in Section~\ref{io_automata} are not lost when one considers that the only actions that modify $A$ and $B$ must be serialized, i.e., not executed concurrently with other actions.
