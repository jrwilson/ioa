...
For the framework to be an valid implementation of the I/O automata model, the controller used by the scheduler must be \emph{fair} as defined in Section~\ref{component_model}.
...
The ioa++ framework is designed so that all native concurrency structures (e.g., threads, mutexes) are encapsulated in the dispatcher and controller.
...
A more focused approached used in our framework is to allow automata to declare \emph{dynamically} the actions they would like the scheduler to consider.
...
Not scheduling an (enabled) action could be another potential mistake.
