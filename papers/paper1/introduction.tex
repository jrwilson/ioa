av\section{Introduction\label{introduction}}
...
Thus, we require a component framework for developing asynchronous and concurrent modules.

While a number of component frameworks for distributed systems have been developed, each framework uses threads, event loops, or a combination of both.
...
We observe that locks also complicate composition as the locking strategies of each component must be taken into account when composing to avoid currency hazards.
...
One challenge when using events is the lack of common semantics for how events are generated, distributed, and consumed; thus complicating composition.
...
The alternative is to run both event loops in independent threads and thus face all of the challenges associated with multiple threads discussed previously.
...
I/O automata have independent state, well-defined interfaces, and well-defined interactions under composition and are thus a good foundation for components.
This work extends the I/O automata model for dynamics which is necessary when implementing (as opposed to modeling) distributed systems.
Dynamic composition also facilitates true concurrent execution which is important given the trend toward multi-core platforms.
ioa++ is implemented in C++ for POSIX environments and is available at \url{http://jrwilson.github.com/ioa/}.

\paragraph*{Paper structure}
In Section~\ref{component_model}, we present I/O automata as a suitable foundation for components, make some practical considerations for developing programs with I/O automata, and introduce features for dynamic systems.
