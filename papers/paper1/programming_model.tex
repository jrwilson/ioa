...
A common idiom is to schedule all local actions using one member function where each action is guarded by its precondition as described in section~\ref{scheduling}.
Another common idiom is to have all scheduling functions dispatch to a single scheduling function to avoid replicating scheduling logic.
\ifjournal
