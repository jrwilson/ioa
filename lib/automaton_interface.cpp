...
namespace ioa {
  ...
  automaton_interface::~automaton_interface () {
    ...
    for (std::set<bind_helper_interface*>::const_iterator pos = m_bind_send.begin ();
	 pos != m_bind_send.end ();
	 ++pos) {
      (*pos)->unbound ();
    }
    for (std::set<bind_helper_interface*>::const_iterator pos = m_bind_recv.begin ();
	 pos != m_bind_recv.end ();
	 ++pos) {
      (*pos)->unbound ();
    }
    for (std::set<bind_helper_interface*>::const_iterator pos = m_unbind_send.begin ();
	 pos != m_unbind_send.end ();
	 ++pos) {
      (*pos)->unbound ();
    }
    for (std::set<bind_helper_interface*>::const_iterator pos = m_unbind_recv.begin ();
  ...
  void automaton_interface::create (automaton_helper_interface* helper) {
    assert (helper != 0);
  ...
  void automaton_interface::bind (bind_helper_interface* helper) {
    assert (helper != 0);
  ...
  void automaton_interface::unbind (bind_helper_interface* helper) {
    assert (helper != 0);
    ...
    std::set<bind_helper_interface*>::const_iterator send_iter = m_bind_send.find (helper);
    std::set<bind_helper_interface*>::const_iterator recv_iter = m_bind_recv.find (helper);
  ...
  void automaton_interface::destroy (automaton_helper_interface* helper) {
    assert (helper != 0);
  ...
  SYSTEM_OUTPUT_DEF (automaton_interface, sys_create, std::pair<shared_ptr<generator_interface> COMMA void*>) {
  ...
  SYSTEM_OUTPUT_DEF (automaton_interface, sys_bind, std::pair<shared_ptr<bind_executor_interface> COMMA void*>) {
    std::pair<bool, std::pair<shared_ptr<bind_executor_interface>, void*> > retval;
    
    if (sys_bind_precondition ()) {
      std::set<bind_helper_interface*>::iterator pos = m_bind_send.begin ();
      bind_helper_interface* helper = *pos;
  ...
  SYSTEM_OUTPUT_DEF (automaton_interface, sys_unbind, void*) {
    std::pair<bool, void*> retval;
    
    if (sys_unbind_precondition ()) {
      std::set<bind_helper_interface*>::iterator pos = m_unbind_send.begin ();
      bind_helper_interface* helper = *pos;
  ...
  SYSTEM_OUTPUT_DEF (automaton_interface, sys_destroy, void*) {
    std::pair<bool, void*> retval;
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_create_key_exists, void*, t) {
    // We prevent this in create.
    assert (false);
  }

  SYSTEM_INPUT_DEF (automaton_interface, sys_instance_exists, void*, t) {
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_automaton_created, std::pair<void* COMMA aid_t>, t) {
    // Find the helper (sanity check).
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_bind_key_exists, void*, t) {
    // We prevent this in bind.
    assert (false);
  }
  
  SYSTEM_INPUT_DEF (automaton_interface, sys_output_automaton_dne, void*, t) {
    // Find the helper (sanity check).
    std::set<bind_helper_interface*>::const_iterator pos = m_bind_recv.find (static_cast<bind_helper_interface*> (t));
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_input_automaton_dne, void*, t) {
    // Find the helper (sanity check).
    std::set<bind_helper_interface*>::const_iterator pos = m_bind_recv.find (static_cast<bind_helper_interface*> (t));
    assert (pos != m_bind_recv.end ());
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_binding_exists, void*, t) {
    // Find the helper (sanity check).
    std::set<bind_helper_interface*>::const_iterator pos = m_bind_recv.find (static_cast<bind_helper_interface*> (t));
    assert (pos != m_bind_recv.end ());
    (*pos)->binding_exists ();
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_output_action_unavailable, void*, t) {
    // Find the helper (sanity check).
    std::set<bind_helper_interface*>::const_iterator pos = m_bind_recv.find (static_cast<bind_helper_interface*> (t));
    assert (pos != m_bind_recv.end ());
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_input_action_unavailable, void*, t) {
    // Find the helper (sanity check).
    std::set<bind_helper_interface*>::const_iterator pos = m_bind_recv.find (static_cast<bind_helper_interface*> (t));
    assert (pos != m_bind_recv.end ());
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_bound, void*, t) {
    // Find the helper (sanity check).
    std::set<bind_helper_interface*>::const_iterator pos = m_bind_recv.find (static_cast<bind_helper_interface*> (t));
    assert (pos != m_bind_recv.end ());
    (*pos)->bound ();
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_bind_key_dne, void*, t) {
    // We prevent this in unbind.
    assert (false);
  }
  
  SYSTEM_INPUT_DEF (automaton_interface, sys_unbound, void*, t) {
    // Something was unbound.
    // It can be in m_bind_recv, m_unbind_send, or m_unbind_recv.
    
    bind_helper_interface* helper = static_cast<bind_helper_interface*> (t);
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_create_key_dne, void*, t) {
    // We prevent this in destroy.
    assert (false);
  }

  SYSTEM_INPUT_DEF (automaton_interface, sys_automaton_destroyed, void*, t) {
    // An automaton was destroyed.
    // It can be in m_create_recv, m_destroy_send, or m_destroy_recv.
    
    automaton_helper_interface* helper = static_cast<automaton_helper_interface*> (t);
  ...
  SYSTEM_INPUT_DEF (automaton_interface, sys_recipient_dne, void*, t) {
    // TODO
    assert (false);
  }

  SYSTEM_INPUT_DEF (automaton_interface, sys_event_delivered, void*, t) {
    // TODO
    assert (false);
  }
