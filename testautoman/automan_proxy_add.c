...
proxy_generator_proxy_created (void* state, void* param, receipt_type_t receipt)
{
  ...
  assert (automan_proxy_send (pg_proxy->aid, -1, &pg_proxy->proxy_request) == 0);
}
...
static void automaton_proxy_created (void* state, void* param, proxy_receipt_type_t receipt);
...
static void
automaton_proxy_created (void* state, void* param, proxy_receipt_type_t receipt)
{
  automaton_t* automaton = state;
  assert (automaton != NULL);
