...
static void automaton_composed1 (void* state, void* param, receipt_type_t receipt);
static void automaton_composed2 (void* state, void* param, receipt_type_t receipt);
...
static void
automaton_composed1 (void* state, void* param, receipt_type_t receipt)
{
  automaton_t* automaton = state;
  assert (automaton != NULL);

  if (receipt == COMPOSED) {
    assert (automaton->composed1);
  }
  else if (receipt == OUTPUT_UNAVAILABLE) {
    assert (!automaton->composed1);
    exit (EXIT_SUCCESS);
  }
}
...
static void
automaton_composed2 (void* state, void* param, receipt_type_t receipt)
{
  automaton_t* automaton = state;
  assert (automaton != NULL);

  if (receipt == COMPOSED) {
    assert (automaton->composed2);
  }
  else if (receipt == OUTPUT_UNAVAILABLE) {
    assert (!automaton->composed2);
    exit (EXIT_SUCCESS);
